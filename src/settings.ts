...
export class SettingTab extends PluginSettingTab {
	...
	private async renderPublications(containerEl: HTMLElement): Promise<void> {
		...
		if (!this.plugin.settings.identifier || !this.plugin.settings.appPassword) {
			containerEl.createEl("p", { text: "Set your handle and app password to load publications." });
			return;
		}

		const loading = containerEl.createEl("p", { text: "Loading publications..." });
		try {
			const resp = await getPublications(this.plugin.client, this.plugin.settings.identifier);
			loading.remove();
